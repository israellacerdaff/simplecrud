...
def get_users():
  try:
    ...
    return make_response(jsonify({'message': 'erro ao buscar usuário', 'error': str(e)}), 500)
  
# puxa os users pelo id
...
def update_user(id):
  try:
    ...
    if user:
      ...
      db.session.commit()
      return make_response(jsonify({'message': 'usuário criado'}), 200)
    return make_response(jsonify({'message': 'usuário não encontrado'}), 404)  
  except Exception as e:
      return make_response(jsonify({'message': 'erro ao subir usuário', 'error': str(e)}), 500)
...
def delete_user(id):
  try:
    ...
    return make_response(jsonify({'message': 'erro ao deletar usuário', 'erro': str(e)}), 500)   